...
const BookingReport = () => {
 ...
 const generateCSV = () => {
  ...
  const csvData = [
    ...
    [
      "Reporting Period:",
      selectedPeriod === "custom" ? getPeriodDisplayText() : selectedPeriod.toUpperCase(),
    ],
    [
      "Hotel Filter:",
    ...
    [
      ...
      `${summary.totalBookingsGrowth > 0 ? '+' : ''}${summary.totalBookingsGrowth}%`,
      summary.totalBookingsGrowth > 0 ? "Growing" : summary.totalBookingsGrowth < 0 ? "Declining" : "Stable"
    ],
    [
      "Confirmed Bookings",
      summary.confirmedBookings.toLocaleString(),
      `${summary.confirmationRate}% rate`,
      summary.confirmationRate > 80 ? "Excellent" : summary.confirmationRate > 60 ? "Good" : "Needs Attention"
    ],
    [
      "Cancelled Bookings",
      summary.cancelledBookings.toLocaleString(),
      `${summary.cancellationRate}% rate`,
      summary.cancellationRate < 10 ? "Excellent" : summary.cancellationRate < 20 ? "Acceptable" : "High Risk"
    ],
    [
      "Average Booking Value",
      formatCurrency(summary.avgBookingValue),
      `${summary.avgBookingValueGrowth > 0 ? '+' : ''}${summary.avgBookingValueGrowth}%`,
    ...
    ["Month", "Total Bookings", "Revenue Generated", "Month-over-Month Growth"],
  ];
  ...
  monthlyTrends.forEach((trend, index) => {
    ...
    const bookingGrowth = prevMonth 
      ? (((trend.bookings - prevMonth.bookings) / prevMonth.bookings) * 100).toFixed(1)
      : "N/A";
  ...
  csvData.push([""], ["=== DETAILED BOOKING TRANSACTIONS ==="],
  [
    "Booking ID",
    "Guest Name",
    "Contact Number",
  ...
  recentBookings.forEach((booking) => {
    ...
    const revenueImpact = booking.status.toLowerCase() === 'cancelled' ? 'Lost Revenue' : 
                         booking.status.toLowerCase() === 'confirmed' ? 'Confirmed Revenue' : 'Pending Revenue';
  ...
  csvData.push(
    ...
    ["Confirmed Revenue:", formatCurrency(recentBookings.filter(b => b.status.toLowerCase() === 'confirmed').reduce((sum, b) => sum + b.amount, 0))],
    ["Potential Lost Revenue:", formatCurrency(recentBookings.filter(b => b.status.toLowerCase() === 'cancelled').reduce((sum, b) => sum + b.amount, 0))],
    ["Average Transaction Value:", formatCurrency(recentBookings.reduce((sum, b) => sum + b.amount, 0) / recentBookings.length)],
    [""],
...
const generateExcel = () => {
  ...
  const executiveSummary = [
    ...
    ["Reporting Period:", selectedPeriod === "custom" ? getPeriodDisplayText() : selectedPeriod.toUpperCase()],
    ["Hotel Scope:", hotels.find((h) => h._id === selectedHotel)?.hotelName || "All Hotels"],
    [""],
    ["KEY PERFORMANCE INDICATORS"],
    ["Metric", "Value", "Growth Rate", "Performance Rating"],
    ["Total Bookings", summary.totalBookings, `${summary.totalBookingsGrowth}%`, summary.totalBookingsGrowth > 0 ? "↗ Growing" : "↘ Declining"],
    ["Confirmation Rate", `${summary.confirmationRate}%`, "", summary.confirmationRate > 80 ? "★★★ Excellent" : "★★ Good"],
    ["Cancellation Rate", `${summary.cancellationRate}%`, "", summary.cancellationRate < 10 ? "★★★ Low Risk" : "⚠ Monitor"],
    ["Avg Booking Value", summary.avgBookingValue, `${summary.avgBookingValueGrowth}%`, summary.avgBookingValueGrowth > 0 ? "↗ Improving" : "↘ Declining"],
    ["Estimated Total Revenue", summary.totalBookings * summary.avgBookingValue, "", ""],
    [""],
    ["STRATEGIC INSIGHTS"],
    ["Business Health Score:", summary.confirmationRate > 80 && summary.cancellationRate < 15 ? "HEALTHY" : "REQUIRES ATTENTION"],
    ["Revenue Trend:", summary.avgBookingValueGrowth > 0 ? "POSITIVE GROWTH" : "DECLINING - ACTION NEEDED"],
    ["Market Position:", summary.totalBookingsGrowth > 10 ? "EXPANDING" : summary.totalBookingsGrowth > 0 ? "STABLE GROWTH" : "MARKET CONTRACTION"]
  ];
  ...
  const performanceData = [
    ...
    [""],
    ["Month", "Bookings", "Revenue", "Booking Growth %", "Revenue Growth %", "Performance Status"]
  ];

  monthlyTrends.forEach((trend, index) => {
    const prevMonth = monthlyTrends[index - 1];
    const bookingGrowth = prevMonth ? (((trend.bookings - prevMonth.bookings) / prevMonth.bookings) * 100).toFixed(1) : 0;
    const revenueGrowth = prevMonth ? (((trend.revenue - prevMonth.revenue) / prevMonth.revenue) * 100).toFixed(1) : 0;
    const status = parseFloat(bookingGrowth as string) > 5 ? "Strong Growth" : parseFloat(bookingGrowth as string) > 0 ? "Growth" : "Decline";
  ...
  trendsWS['!cols'] = [{wch: 12}, {wch: 12}, {wch: 15}, {wch: 15}, {wch: 15}, {wch: 15}];
  ...
  // Add summary rows
  const confirmedRevenue = recentBookings.filter(b => b.status.toLowerCase() === 'confirmed').reduce((sum, b) => sum + b.amount, 0);
  const lostRevenue = recentBookings.filter(b => b.status.toLowerCase() === 'cancelled').reduce((sum, b) => sum + b.amount, 0);
  ...
  transactionData.push(
    ...
    ["Success Rate:", `${((confirmedRevenue / (confirmedRevenue + lostRevenue)) * 100).toFixed(1)}%`]
  );

  const bookingsWS = XLSX.utils.aoa_to_sheet(transactionData);
  bookingsWS['!cols'] = [
    {wch: 15}, {wch: 20}, {wch: 15}, {wch: 20}, {wch: 15}, 
...
const handleExport = () => {
  ...
  const hotelName = hotels.find((h) => h._id === selectedHotel)?.hotelName?.replace(/\s+/g, '-') || "AllHotels";
  const periodText = selectedPeriod === "custom" ? "Custom-Range" : selectedPeriod;
      ...
      <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <div className="flex-1">
                <div className="font-semibold text-gray-900">CSV Report</div>
            ...
            >
              ...
              <div className="flex-1">
                <div className="font-semibold text-gray-900">Excel Workbook</div>
                <div className="text-sm text-gray-600">
        ...
        <div className="bg-gray-50 p-3 rounded border-l-4 border-gray-400">
          <div className="text-xs text-gray-600">
            <strong>Report Period:</strong> {selectedPeriod === "custom" ? getPeriodDisplayText() : selectedPeriod} <br/>
            <strong>Hotel Filter:</strong> {hotels.find((h) => h._id === selectedHotel)?.hotelName || "All Hotels"} <br/>
            <strong>Total Records:</strong> {recentBookings.length} transactions
          </div>
        </div>
      </div>
