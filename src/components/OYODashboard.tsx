...
import {
  ...
  getDashboardCounts,
  getRevenueVsBookingChart,
} from "@/api/Services/Report/report";
...
const OYODashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const revenueData = await getRevenueVsBookingChart(
        apiPeriod,
        selectedHotels
      );
      if (
      ...
      const bookingsData = await getDailyBookingsChart(
        apiPeriod,
        selectedHotels
      );
      if (
  ...
  const occupancyRate =
    displayData.totalRooms > 0
      ? Math.round(
          ((displayData.totalRooms - displayData.todayVacantRooms) /
            displayData.totalRooms) *
            100
        )
      : 0;
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {selectedPeriod} performance overview
                    </p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                    ...
                    <div className="flex space-x-2 ml-4">
                      <div className="bg-blue-50 rounded-lg px-2 py-1 border border-blue-200">
                        <span className="text-xs text-blue-600 font-medium">
                          Peak Day: {chartSummary.peakDay} (
                          {chartSummary.peakBookings} bookings)
                        </span>
                      </div>
          ...
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-8 shadow-lg border border-purple-100">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600">
                  Daily booking patterns and trends
                </p>
              </div>
                ...
                <div className="flex space-x-2">
                  <div className="bg-white rounded-lg px-3 py-1 shadow-sm border border-purple-200">
                    <span className="text-xs text-purple-600 font-medium">
                      Peak: {bookingAnalytics.peakBookings} bookings (
                      {bookingAnalytics.peakBookingDay})
                    </span>
                  </div>
                  <div className="bg-white rounded-lg px-3 py-1 shadow-sm border border-purple-200">
                    <span className="text-xs text-purple-600 font-medium">
                      Avg: {Math.round(bookingAnalytics.averageBookingsPerDay)}{" "}
                      bookings/day
                    </span>
                  </div>
                  <div className="bg-white rounded-lg px-3 py-1 shadow-sm border border-purple-200">
